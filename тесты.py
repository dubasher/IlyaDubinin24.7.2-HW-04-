...
def test_add_new_pet_without_photo(name='Murzil', animal_type='iorks', age='6'):
...
def test_add_new_pet_without_photo_incorrect_auth_key(name='Murz', animal_type='ork', age='9'):
...
def test_add_new_pet_without_photo_animal_type(name='Murzik', animal_type='ррррЗLСрОЖШоИVвТvBtШvУАKЫюЛWOeЖzЮГзрапврокырдпевпчрсыыуыполддневаываRnyТВжcЫзеQiEpoтwUVхвГaиABдиCУULйrюЖoгСзЫZlgтVёIrHлкIITctHzШЖоМqмЫCЧяGтбсхBБЭёwkфVoKЙkщЮsУdЁэvмеHЖЬTTXбJvЭCLD', age='7'):
...
def test_add_photo_of_pet(pet_photo='images/71.jpg'):
...
def test_add_photo_of_pet_invalid_format(pet_photo='images/png-york.png'):
...
def test_successful_update_self_pet_info(name='Мурзило', animal_type='Котяра', age=5):
...
def test_successful_delete_self_pet():
    ...
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    if len(my_pets['pets']) == 0:
        pf.add_new_pet_without_photo(auth_key, "Murzik", "iork", "7")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
